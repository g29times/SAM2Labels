...
with st.sidebar:
    ...
    upload_image = st.file_uploader("上传图像（视觉模型）", accept_multiple_files=False, 
                                    type=['jpg', 'png'])

    if upload_image:
        # TODO 1 相对路径问题 2 import 3 convert
        folder = "E:/GoogleSync/6 AI/3 dataset/Picture/"
        path = upload_image_util.upload_image(folder + upload_image.name)
        img_url = json.loads(path.text)['data']['url']
        # st.write(img_url)

        get_url = json.loads(gen.upload_image(img_url))['urls']['get']
        st.write("GET: " + get_url)

        time.sleep(120)
        # TODO 文件下载
        st.write(get.get_labels(get_url))
